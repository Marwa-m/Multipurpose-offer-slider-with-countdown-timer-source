...
namespace Nop.Plugin.Widgets.DailyOffers.Services.Offers
{
    public partial class OfferService : IOfferService
    {
        ...
        public virtual void InsertOffer(Offer offer)
        {
            ...
            //cache
            _cacheManager.RemoveByPattern(OFFERS_PATTERN_KEY);
            _cacheManager.RemoveByPattern(PRODUCTOFFERS_PATTERN_KEY);
            _cacheManager.RemoveByPattern(WIDGETZONEOFFERS_PATTERN_KEY);
            _cacheManager.RemoveByPattern(OFFERPRODUCTCATEGORY_MAINBYPRODUCTID_KEY);
            ...
            if ((storeId > 0 ) || (!showHidden ))
            {
                if (!showHidden )
                {
                    ...
                    var ACLQry = from acl in _aclRepository.Table
                                   where (acl.EntityName == "Offer" && allowedCustomerRolesIds.Contains(acl.CustomerRoleId))
                                   select acl.EntityId;
                ...
                if (storeId > 0 )
                {
                    ...
                    var storeQry = from sm in _storeMappingRepository.Table
                                   where (sm.EntityName == "Offer" && storeId == sm.StoreId)
                                   select sm.EntityId;
       ...
       public IList<Offer> GetOffersByWidgetZoneName(string WidgetName = "")
        {
            ...
            if (_storeContext.CurrentStore.Id > 0 )
            {
                    ...
                    //Store mapping
                    var storeQry = from sm in _storeMappingRepository.Table
                                   where (sm.EntityName == "Offer" && _storeContext.CurrentStore.Id == sm.StoreId)
                                   select sm.EntityId;
                    var storeMap = storeQry.ToList();
